...
suite('Unit Tests', function () {
  ...
  assert.notStrictEqual(6, '6', '6 no es estrictamente igual a "6"');
  assert.strictEqual(6, 3 * 2, '6 es estrictamente igual a 3 * 2');
  assert.strictEqual(6 * '2', 12, '6 * "2" se convierte en el número 12');
  assert.notStrictEqual([1, 'a', {}], [1, 'a', {}], 'dos arrays iguales en contenido no son la misma referencia');
});
    // #7
  ...
  assert.isBelow('world'.length, 6, "5 es menor que 6");
  assert.isAtLeast(2 * Math.random(), 0, "2 * random siempre es >= 0");
  assert.isBelow(5 % 2, 2, "5 % 2 es 1, y 1 < 2");
  assert.isAtLeast(2 / 3, 0.5, "2/3 es mayor o igual que 0.5");
});
    // #10
